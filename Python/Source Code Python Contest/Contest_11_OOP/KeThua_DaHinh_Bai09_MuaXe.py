class Vehicle:
    ...
    # Cách 1:
    def is_price_in_range(self, price_start: int, price_end: int) -> bool:
        return price_start <= self._price <= price_end

    # Cách 2:
    #   return (self._price >= price_start) and (self._price <= price_end)
...
def main() -> None:
    ...
    price_start, price_end = map(int, input().split())

    print("DANH SACH OTO :")
    for car in list_cars:
        if car.is_price_in_range(price_start, price_end):
            print(car)

    print("DANH SACH XE MAY :")
    for motorbike in list_motorbikes:
        if motorbike.is_price_in_range(price_start, price_end):
            print(motorbike)
